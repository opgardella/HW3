...
## HW 3
## Olivia Gardella
## Worked with Lauren in discussion
...
# Basic Flask application setup (added)
...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://postgres:si364@localhost/gardellaHW3"
## Provided:
...
class TweetForm(FlaskForm):
    text = StringField("Enter the text of the tweet (no more than 280 characters):", validators=[Required(),Length(1,280)])
    username = StringField("Enter username of the twitter user (no '@'):",validators=[Required(),Length(1,64)])
...
# TODO 364: Make sure to check out the sample application linked in the readme to check if yours is like it!
    ...
    def validate_username(self,field):
        username = field.data
        if username[0] == '@':
            raise ValidationError("Username may not start with '@'!")

    def validate_display_name(self,field):
        display_name = field.data
        display_name_split = display_name.split(' ')
        if len(display_name_split) < 2:
            raise ValidationError("Display name should be at least 2 words!")
...
def get_longest_tweet():
    ...
    max_tweet_lst = []
    for t in tweets:
        if (len((t.text).replace(" ","")) > max_len) or (len((t.text).replace(" ","")) == max_len and (t.text)[0] < max_tweet[0]):
            max_len = len((t.text).replace(" ",""))
            print(max_len)
            max_tweet = t.text
            user = User.query.filter_by(id=t.user_id).first()
            display_name = user.display_name
            username = user.username
    return render_template('longest_tweet.html',username=username, text=max_tweet, display_name=display_name)
